...
**Status atual:** os testes são **planejados e não executados**, pois este projeto documental não possui uma implementação funcional disponível no Microsoft Copilot Studio.
...
Quando os testes forem efetivamente executados, recomenda-se registrar:
...
Não foram executados porque este projeto documental não possui acesso ao ambiente original do Microsoft Copilot Studio utilizado para a implementação do cenário.

Consequentemente, este documento não apresenta métricas de desempenho, taxa de acerto ou resultados funcionais reais.

## 6.8 Rastreabilidade

Os casos de teste estão relacionados aos requisitos definidos em [`docs/05-requisitos/README.md`](../05-requisitos/README.md):
...
## 6.9 Classificação das informações
